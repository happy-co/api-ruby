...
puts "Attempting archive asset request with client_id: #{client_id} and secret: #{client_secret}."
...
request4 = Happyco::Manage::Asset::V1::SetAssetsArchivedRequest.new(
  account_id: response.accounts.first.id, 
  folder_id: response2.folders.first.id,
  asset_ids: [response3.assets.first.id]
)
response4 = stub3.archive_assets(request4)
