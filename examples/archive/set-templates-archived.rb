...
puts "Attempting archive template request with client_id: #{client_id} and secret: #{client_secret}."
...
response3 = stub3.list_templates(request3)

request4 = Happyco::Inspect::Template::V1::SetTemplatesArchivedRequest.new(
  account_id: response.accounts.first.id,
  folder_id: response2.folders.first.id,
  template_ids: [response3.templates.first.id],
)
response4 = stub3.archive_templates(request4)
